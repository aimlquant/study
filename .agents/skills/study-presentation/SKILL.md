...
`materials/<track>/archive/` for a completed one. Keep the public path under
`html/studies/<study-slug>` unchanged across that lifecycle.
...
Preserve the source's learning coordinates before reorganizing its ideas. Use the source chapter and subsection order as the report's primary spine, and keep visible source numbers and translated titles for every chapter section, figure, table, listing, example, box, and exercise that the source contains. Do not replace `3.3.2 Annotation ingestion and processing` with an unrelated report-local title or merge `Example 5.1–5.5` into unnamed prose. Put interpretation, reproduction audits, operational safeguards, and decision guidance inside the matching source section or in visibly labeled AIML Quant supplements after the source outline. Never move a supplement into the source's numbering namespace.

Link every textbook file through the invitation page, never directly to GitHub. Learning materials live in the private repository named by `materials_repository` in `agent-support/site.toml`, so a raw `https://github.com/<owner>/<repo>/blob/...` link returns GitHub's 404 to everyone outside the organization. Write `../../../../materials/?p=<path under the materials repository>` instead, dropping the leading `materials/` segment: a link to `materials/aiml/active/kg-llm/ch03/guide.md` becomes `../../../../materials/?p=aiml/active/kg-llm/ch03/guide.md`. The public site answers that path with the requested file name, the participation notice, and the members' entry point into the private repository. `source_material` in `presentation.toml` keeps the registry-relative path and is not rewritten.

For every new or substantially revised report, add a visible source map that links each source coordinate and exact title to its report anchor. Set `source_fidelity = "source-outline-v1"` and `source_material` in `presentation.toml` only after the map is complete. Keep report-created asset numbering separate from source numbering, and label reconstructed visuals with both identities, for example `리포트 그림 4 · 교재 그림 3.1–3.2 해설`. Avoiding long verbatim text or copied artwork does not justify deleting or renaming navigational metadata.
...
Treat the report as a source-aligned long-form publication, not a slide transcript. Preserve the canonical ConnectBrick-derived component hierarchy, but do not let the blueprint's problem-to-decision logic replace or reorder the book's outline. Use that logic as a coverage checklist within the matching source sections and supplements. Add source-backed tables and newly composed SVG diagrams when they materially explain the chapter. Give report sections, tables, and figures stable IDs, and mark report figures that the deck must carry with `data-deck-use="required"`. The report gate requires complete source-map coverage, complete argument coverage, captions and sources, working TOC/lightbox, desktop/mobile rendering, and an inspected A4 PDF.

Derive the deck's narrative, claims, terminology, tables, SVGs, and CSS relationships from the approved report. Keep conclusion-style slide titles, but show the corresponding source section, figure, table, listing, or example coordinate on the slide and in `data-source-refs`; a listener must be able to find the book location without searching the report. Reuse a report visual directly when it is legible at slide scale; otherwise make a faithful slide-scale adaptation without changing its meaning. Keep `data-report-source="report.html"` on the deck main element and put valid `data-report-refs` on every slide. Preserve automatic report/deck TOCs and verify that every report section, every source section, and every required figure is covered.
