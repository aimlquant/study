...
description: Build, repair, or review reproducible Python chapter experiments for Ernest P. Chan book projects, including run_chapterX_analysis.py, executed Jupyter notebooks, official data and source provenance, Markdown reports, figures, and book-result comparisons. Use when asked to implement or improve a chapter analysis, experiment notebook, backtest report, or execution environment for an Ernest P. Chan textbook, including the Quant track's `materials/quant/active/machine-trading`.
---
...
Before creating or substantially revising a chapter, inspect the strongest existing chapter artifacts available. In this repository the peer set is the other chapters of the same textbook, because only one Chan book is present; when a sibling Chan book is available, include it. Compare at least:
...
Run the audit on the target and candidates from the textbook's locked environment, because the script imports the book's shared module:

```bash
uv run --project materials/quant/active/<study-slug> python \
  .agents/skills/create-chan-chapter-analysis/scripts/audit_chapter_artifacts.py \
  materials/quant/active/<study-slug>/chapter_N_*/src/chapterN_full_report.ipynb
```
 Treat its scores as a repeatable baseline, not a substitute for judgment. Deep-read the leading candidates and manually check numerical claims, result-to-conclusion consistency, truncated experiments, chart legibility, and actual isolated execution.
