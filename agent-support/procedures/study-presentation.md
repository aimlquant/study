...
- 리포트와 발표자료는 각각 화면에 등장하는 순서로 독립적인 `그림 N`, `표 N` 번호를 쓴다. 발표 캡션에 `상세 리포트 그림 N`, `상세 리포트 표 N`, `교재 그림 N`을 자체 번호처럼 붙이지 않는다. 리포트·교재와의 관계는 `data-report-refs`, `data-source-refs`, 정확한 링크와 작은 출처 좌표로 보존한다. 기본 발표에서 선택형 부록을 숨기면 슬라이드 번호와 그림 번호를 현재 모드 기준으로 다시 계산한다. 발표자료의 `그림 N` 은 두 층에서 정해진다. 독자가 보는 값은 `deck.js` 의 `numberFigures()` 가 슬라이드 순서대로 `figcaption > b` 를 런타임에 덮어써 만들고, 검증기는 정적 HTML 의 `<b>그림 N</b>` 이 그 슬라이드 `data-report-refs` 대상의 **리포트 캡션 번호**와 같기를 요구한다. 두 값이 어긋나지 않게 하려면 **덱의 그림 슬라이드 순서를 리포트의 그림 순서와 같게 두고** 정적 `<b>` 에 리포트 번호를 적는다. 리포트에만 있는 그림을 중간에 끼우면 이 순서가 깨지므로, 그 그림을 덱에도 넣거나 리포트에서 위치를 옮긴다. 리포트 자산을 재번호했으면 덱의 `<b>` 도 함께 고친다.
