...
- `materials_path` 아래의 원문·번역·해설·노트·코드·기존 도형을 읽고 `원본 장·절 순서 / 절 번호·제목 / 그림 번호·제목 / 표 번호·제목 / Listing·Example·Box·연습문제 번호·제목 / 핵심 주장 / 주장 주체·귀속 / 근거·예제 / 비교 기준 / 조건·단서 / 한계 / 시점 범위 / 용어 / 시각화할 관계 / 출처 위치`를 먼저 목록화한다. 이 원자료 감사가 리포트의 입력이다.
...
- 프롬프트, 청사진 지시문, 생성 파이프라인, 규약 준수 설명은 제작자용 정보다. 공개 리포트와 발표자료의 표지·개요·목차·요약에서 `SOURCE → REPORT → PRESENTATION`, “원본 순서를 따른다”, “보충은 마지막” 같은 제작 과정을 메시지로 삼지 않는다. 같은 자리는 독자가 실제로 배워야 할 질문·설계 계약·작동 원리·근거·한계로 교체하고, 원본 좌표는 간결한 탐색·출처 정보로만 유지한다.
