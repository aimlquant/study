...
- 교재의 핵심 관계·흐름·비교가 시각화에 적합하면 원본을 복제하지 않은 발표자 도해를 `assets/figs/`에 만든다. 캡션에는 리포트 자체 번호와 대응하는 `그림 N · 원본 제목`을 함께 표시한다. `교재 그림 N`처럼 불필요한 접두어를 반복하지 않고, 원본 좌표는 `data-source-ref`와 가까운 출처에서 보존한다. 시각자료는 원본 자산과 학습 질문에 대응시킨다. 개수는 합격 기준이나 상한이 아니며, 관계·차이·시간 변화·계산 과정을 실제로 설명하는 데 필요한 만큼 사용한다.
...
- 리포트와 발표자료는 각각 화면에 등장하는 순서로 독립적인 `그림 N`, `표 N` 번호를 쓴다. 발표 캡션에 `상세 리포트 그림 N`, `상세 리포트 표 N`, `교재 그림 N`을 자체 번호처럼 붙이지 않는다. 리포트·교재와의 관계는 `data-report-refs`, `data-source-refs`, 정확한 링크와 작은 출처 좌표로 보존한다. 기본 발표에서 선택형 부록을 숨기면 슬라이드 번호와 그림 번호를 현재 모드 기준으로 다시 계산한다. 새 덱은 main에 `data-caption-scope="deck"`, 자체 캡션 라벨에 `data-deck-caption="figure"` 또는 `"table"`을 둔다. 정적 라벨도 덱 순서의 독립 번호를 쓴다. 번호가 적힌 `report-ref` 링크는 해당 리포트의 번호와 대조한다. 이 구분 없이 발행된 기존 덱은 기존 검사를 유지한다. 번호 때문에 리포트 그림을 이동하거나 불필요한 그림을 덱에 추가하지 않는다.
