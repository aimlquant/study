...
- 상세 리포트는 실제 `study-report-v1`의 `ai-odyssey-publisher` 계열 컴포넌트(`report-section`, `section-summary`, `cmp-table`, `report-figure`, `callout--*`)로 작성한다. 표지만 닮은 별도 경량 CSS나 카드 모음으로 대체하지 않는다.
- 교재의 핵심 관계·흐름·비교가 시각화에 적합하면 원본을 복제하지 않은 발표자 도해를 `assets/figs/`에 만든다. 리포트 캡션에는 자체 번호와 대응하는 `그림 N · 원본 제목`을 함께 표시한다. `교재 그림 N`처럼 불필요한 접두어를 반복하지 않고, 원본 좌표는 `data-source-ref`와 가까운 출처에서 보존한다. 시각자료는 원본 자산과 학습 질문에 대응시킨다. 개수는 합격 기준이나 상한이 아니며, 관계·차이·시간 변화·계산 과정을 실제로 설명하는 데 필요한 만큼 사용한다.
...
4. 작성 시작부터 `presentation.toml`에 `source_fidelity = "source-structure-v1"`과 저장소 상대 경로 `source_material`을 기록한다.
...
새 회차는 `--artifacts report`로 리포트만 만들 수 있다. `--source-material`에는 원본 목차를 대표할 한국어 번역 또는 해설 경로를 전달한다. 원서와 가장 일치하는 제목 체계를 고르고, 숫자 없는 하위 제목도 원자료 감사에서 보존한다. 경로를 비워 만든 초안은 발행 검증을 통과하지 않는다. 운영 논의 등 교재 장 해설이 아닌 문서는 그 목적과 별도 검토 근거를 명시하고 해당 없는 교재 계약을 적용하지 않는다.

새 교재 리포트에는 `report_quality = "source-learning-v1"`와 `content_review = "agent-support/reviews/<회차>-content.json"`을 둔다. 기록은 제작자용이며 원문 전문을 복사하지 않는다. JSON 계약:

- `schema_version: 1`, `verdict: "reviewed"`, `report_sha256`: 마지막으로 내용 검토한 HTML의 SHA-256.
- `sources`: `role`(최소 `original`, `outline`; 필요하면 `notebook`, `errata`), 교재 경로 `path`, `sha256`. `outline`은 `source_material`과 같아야 한다. 원서가 1차 근거이고 확인된 정오는 근거와 함께 구분한다.
  Git으로 관리하는 원자료는 검토한 파일이 속한 `revision` 커밋도 기록하고 그 커밋의 파일 바이트와 해시가 일치하는지 확인한다. 다른 작업자의 미커밋 변경을 대신 커밋하지 않는다.
- `sections`: 원본 절마다 `ref`, 실제 본문 `report_id`, `explanation`(주장과 중간 설명), `evidence`(원서 사례·수치·논증과 반영 위치), `limits`(조건·생략·자체 보충), `visuals`(해결하는 질문과 도형 위치). 적용되지 않는 항목은 이유를 쓴다.
- `experiments`: `status`(`executed`, `stored-output`, `book-only`, `not-run`), `scope`, 셀 ID·함수와 실행 증거를 찾을 `source`, 실제 출력·코드의 `report_ids`, `interpretation`. 같은 결과를 문서 다른 곳에서 다른 실행 상태로 설명하지 않는다.

자동 검사는 기록의 완결성·경로·해시·본문 연결을 확인한다. 핵심 설명이 실제로 남았는지, 원문에 없는 단정이나 수치가 들어갔는지는 원문 대조와 동료 검토에서 별도로 판정한다. 기록을 채운 사실만으로 내용 충분 판정을 내리지 않는다. 기존 발행물은 기존 검토 절차를 유지하며, 재작성할 때 이 계약을 적용한다.

노트북은 목적→데이터와 시간 조건→핵심 코드→실제 출력→읽는 법→한계를 한 실험으로 연결한다. 원서에 실린 수치, 공식 저장 출력, 이번 실행, 다른 데이터로 만든 개념 실험을 구별한다. 실행하지 않은 코드는 그 상태를 밝히고 가상 출력을 붙이지 않는다. 출력의 확률·단위·표본 범위와 예측 시점을 설명하며, 상태 분류 확률을 수익 확률로 바꾸지 않는다. 짧은 추가 예제는 ‘이해를 위한 예’로 표시하되 모든 문장에 제작 분류를 반복하지 않는다.

### 초보자용 개념·기술 해설

코드가 없는 개념 장에서는 필요한 용어의 첫 등장에 `한 문장 정의 → 왜 필요한가 → 일상 비유와 비유의 한계 → 가까운 개념과의 경계 → 흔한 오해 → 조건·실패 경계`를 가능한 범위에서 제공한다. 전이 학습·온톨로지·추론처럼 추상적인 개념은 최소 한 개의 구체적 입력과 결과를 따라가며 설명하고, 정의 없이 비유나 약어만 먼저 제시하지 않는다. 반복해서 쓰는 전문 용어는 첫 등장 설명과 용어집을 모두 제공하며, 용어집만으로 앞선 무정의 사용을 대신하지 않는다.
...
리포트는 교재에 충실하면서 혼자 읽어도 이해되는 상세 학습 문서다. 위의 귀속 요구를 반복 충족하려다 `교재는 …한다`가 기본 문형이 되면, 일반 기법이 특정 책의 선택으로 축소되고 리포트가 독후감처럼 읽힌다. 귀속을 없애서가 아니라 **붙이는 자리를 바꿔서** 해결한다.
...
- 장·주요 절이 바뀔 때 `chapter-divider` 구분 장을 둔다. 번호와 정확한 번역 제목을 화면 중앙의 한 묶음으로 배치하고 제목이 주인공이 되게 한다. 제목은 상단에 작게 두고 본문에는 큰 숫자만 남기는 배치를 쓰지 않는다. 실제 `h1`을 유지하며 가장 긴 제목을 포함한 모든 구분 장을 데스크톱·모바일에서 확인한다. 구분 장의 참조만으로 설명 커버리지를 채우지 않는다.
...
- 낯선 용어는 처음 필요한 슬라이드에서 한 문장으로 정의한다. 그림 슬라이드는 읽는 순서·실제 관계나 관찰·해석 경계를 보이는 본문으로 설명한다. 제목과 출처만으로 끝내지 않으며, 필요하면 명시적으로 연결한 후속 화면에서 설명한다.
...
- 발표자료의 그림 바로 밑에는 `그림 N | 제목` 형태의 한 줄 표·띠·보이는 `figcaption`을 넣지 않는다. main에 `data-caption-scope="deck" data-figure-captions="none"`을 두고 figcaption 요소를 제거한다. 그림의 정체성은 구체적인 alt와 필요한 경우 figure의 `data-deck-figure-title`로 남긴다. 제목·읽는 법·단위·출처·조건이 캡션에만 있으면 먼저 슬라이드의 편집 가능한 제목·본문·근접 출처로 옮긴다. `data-report-refs`, `data-source-refs`와 정확한 리포트 링크는 유지한다. 이 선호는 발표자료에 적용하며 상세 리포트의 캡션과 실제 수치를 비교하는 표는 유지한다.
- 실제 데이터 표는 덱 자체 순서로 독립적인 `표 N` 번호와 `data-deck-caption="table"`을 쓴다. 사용자가 그림 번호를 명시적으로 요청하면 caption-free 정책을 제거하고 `data-deck-caption="figure"`를 덱 순서대로 붙인다. 리포트 번호가 적힌 링크는 대상 리포트의 번호를 유지한다.
...
- 각 주요 섹션에서 원서의 핵심 주장, 필요한 중간 원리, 구체적 근거·예제와 조건이 실제 본문에 연결되는지 확인한다. 원서에 없는 요소는 억지로 채우지 않고 해당하지 않는 이유를 검토 기록에 남긴다. 짧은 카드와 목록만으로 상세 리포트를 끝내지 않는다.
...
- 상세 리포트의 표·그림에는 번호·제목을 붙인다. 발표자료의 그림 캡션은 §5의 생략 규칙을 따르며, 구체적인 대체 텍스트와 가까운 본문·링크의 재구성 범위·출처를 유지한다.
...
--check-materials는 레지스트리에 등록된 교재 디렉터리가 로컬에 없으면 실패시킨다. 교재가 없을 때 원문 대조가 건너뛰어지는 상태를 발행 검증으로 받아들이지 않기 위한 옵션이다.
...
이미 발행된 리포트에서 덱을 만들 때에는 **리포트의 전체 그림·표**를 먼저 대조한다. 자산별 실제 사용 슬라이드 또는 내용상 생략 이유를 기록한 뒤, 장의 논증과 실험 결과에 필요한 그림을 `data-deck-use="required"`로 표시한다. 작성자가 먼저 고른 required 목록만 검사하면 그 목록에서 빠뜨린 그림을 발견할 수 없다. 리포트 단독 완성은 덱 커버리지를 검증한 상태가 아니며, 그림의 최소 개수를 보편 규칙으로 강제하지 않는다.

새 교재 덱에는 `deck_quality = "report-teaching-v1"`와 `deck_review = "agent-support/reviews/<회차>-deck-content.json"`을 둔다. 기록은 발표 화면에 노출하지 않는 제작자용 자료다.

- `schema_version: 1`, `verdict: "reviewed"`, 검토한 `report_sha256`와 `deck_sha256`.
- `units`: 각 설명 묶음의 `question`, `explanation`(정의·중간 추론), `evidence`(실제 예시·표·그림·출력), `conditions`(조건·생략 이유), `report_refs`, `slide_ids`.
- 모든 본문 절과 실제 슬라이드를 대응시키되, 목차·구분 장에 참조만 추가해서 설명을 대신하지 않는다. 한 단위에서 해당 없는 항목은 억지로 만들지 말고 이유를 쓴다.
- 설명 대응 기록은 실제 화면의 문구·사례·그림을 가리킨다. 리포트에는 있지만 화면에는 없는 설명을 기록에 옮겨 적지 않는다. 절 전체를 한 묶음으로 기록해 개별 비교나 입력 전환의 누락을 감추지 않는다. 예컨대 두 개념의 차이를 구체적 사례로 가르치는 대목은 사례를 지운 추상적 정의만으로 충분하다고 판정하지 않는다.
- `experiments`: 리포트 `notebook-example`의 `report_id`, 같은 실험을 참조하는 `code_slides`, `result_slides`, `interpretation_slides`, `interpretation`을 남긴다. 입력·시간 조건부터 코드·실제 출력·해석까지 연속 화면에서 검토한다.

기계 검사는 해시·앵커·대응 범위와 코드·출력 요소의 존재를 확인한다. 덱이 함께 등록된 새 교재 회차는 리포트에도 해당 덱의 상대 링크가 있어야 한다. 링크를 추가했다고 기록하기 전에 실제 anchor와 설정 메뉴를 확인한다. 항목 수·글자 수·설명 span의 존재로 이해를 판정하지 않는다. 최종 검토에서는 발표자 설명 없이 화면을 읽으며 (1) 낯선 대상의 정의 (2) 결론에 이르는 중간 원리 (3) 도형·표·결과에서 실제 볼 부분 (4) 주장과 해석의 조건이 이어지는지 확인한다. 제목·lead·takeaway가 같은 결론을 반복하거나 핵심 의미가 발표자 노트에만 있으면 수정한다.
