...
2. 핵심 요약 뒤에 원본 절·그림·표·Listing·Example·Box·연습문제를 전수 열거한 **원본 학습 좌표**를 두고 각 항목을 실제 리포트 anchor에 연결한다. 대응 자산이 여러 개로 통합되거나 원본을 직접 싣지 않아도 원본 번호와 정확한 제목은 남긴다.
3. 모든 `.report-section`, 원본 대응 지점, 핵심 표와 그림에 회차 안에서 고유하고 안정적인 `id`를 붙인다. 발표에서 반드시 사용할 그림은 `data-deck-use="required"`로 표시한다.
4. 원본 학습 좌표가 완성된 회차는 `presentation.toml`에 `source_fidelity = "source-outline-v1"`과 저장소 상대 경로 `source_material`을 기록한다.
5. 정의·작동 방식·예시·한계·판단 기준과 출처를 충분히 채운 뒤 리포트 목차, 이미지 전체화면/줌, 데스크톱·모바일, A4 PDF를 실제로 확인한다.
6. 원본 좌표가 누락되거나 내용이 빈약하고 시각자료·캡션·출처·실패 경계가 미완성인 상태에서는 발표자료 본문 작성으로 넘어가지 않는다.
