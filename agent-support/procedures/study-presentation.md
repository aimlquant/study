...
이미 발행된 리포트에서 덱을 만들 때에는 `data-deck-use`를 다시 확인한다. 리포트 단독 완성은 덱 커버리지를 검증한 상태가 아니다. 장의 논증과 실험 결과에 필요한 시각자료를 골라 required로 표시한다. 그림의 최소 개수를 보편 규칙으로 강제하지 않으며 선택·생략 이유를 검토 기록에 남긴다.
