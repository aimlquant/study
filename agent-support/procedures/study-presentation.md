...
- 시각 적합성은 양방향으로 판정한다. 넘침이 없어도 주 도형의 작은 글자가 기본 슬라이드 화면에서 읽히지 않거나, 설명되지 않은 빈 공간이 본문을 지배하는데 도형이 작은 채로 남으면 실패다. 전체화면 확대를 기본 가독성의 대체물로 쓰지 않고, `그림 단독 / 세로형 차트 / 그림+표`처럼 콘텐츠 유형별 크기 계약을 두어 임의의 개별 상한이 쌓이지 않게 한다.
