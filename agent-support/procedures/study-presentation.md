...
- 교재의 핵심 관계·흐름·비교가 시각화에 적합하면 원본을 복제하지 않은 발표자 도해를 `assets/figs/`에 만든다. 캡션에는 리포트 자체 번호와 대응하는 `교재 그림 N · 원본 제목`을 함께 표시한다. 한 챕터 리포트는 원칙적으로 의미 있는 표·도형을 합해 6–10개 제공하되, 내용상 불필요하면 숫자를 억지로 채우지 않는다.
