...
- 기준 완성본: `materials/aiml/active/knowledge-graphs-and-llms-in-action/chapter_03_create_your_first_knowledge_graph_from_ontologies/src/ch03/03_chapter_guide.ipynb`
...
cat "materials/<track>/active/<study-slug>/study.toml"
python3 agent-support/scripts/study-verify.py "materials/<track>/active/<study-slug>" --no-urls
```
...
```bash
python3 "materials/<track>/active/<study-slug>/setup_env.py"            # 없는 것만 채운다 (여러 번 실행해도 안전)
python3 "materials/<track>/active/<study-slug>/setup_env.py" --recreate # .venv 를 다시 만든다
```
...
```bash
python3 agent-support/scripts/study-verify.py "materials/<track>/active/<study-slug>" --no-urls   # 매핑 검증 포함
```
...
```bash
python3 agent-support/scripts/study-new-notebook.py "materials/<track>/active/<study-slug>" --list      # 대상 확인
python3 agent-support/scripts/study-new-notebook.py "materials/<track>/active/<study-slug>" ch05 --dry-run
...
- **그림은 attachment 로 내장한다.** 두 방식이 모두 실패한 이력이 있다. HTML
  ...
  ```bash
  python3 agent-support/scripts/study-new-notebook.py "materials/<track>/active/<study-slug>" ch05 --embed
  ```
...
```bash
python3 agent-support/scripts/study-verify.py "materials/<track>/active/<study-slug>"              # 전체
python3 agent-support/scripts/study-verify.py "materials/<track>/active/<study-slug>" --no-urls    # URL 확인 생략
python3 agent-support/scripts/study-verify.py "materials/<track>/active/<study-slug>" --lint       # 정적 검사만
python3 agent-support/scripts/study-verify.py "materials/<track>/active/<study-slug>" --execute    # 재실행 포함
