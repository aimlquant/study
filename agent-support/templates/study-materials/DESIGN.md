...
5. 리스팅 재현 노트북은 `src/` 에 실행할 코드가 있는 챕터에만 만든다. 공식 소스 지도
   노트북은 자체 경량 실험이 있을 때 전 장을 다룰 수 있지만 비교 등급과 미실행 경계를 밝힌다.
...
공식 저장소가 이미 장별 코드·노트북·데이터를 완결된 형태로 제공하고 전수 실행이 너무 무거우면,
교재 폴더에 `notebook-study/` 같은 **책 로컬 어댑터**를 둘 수 있다. 공용 `studykit`을 복사하는
예외가 아니라 그 책의 장 매핑·경량 실험만 담는 어댑터다. 원장은 장↔공식 소스·질문·데이터·
실험·한계를, 하네스는 결정적 실험과 감사 로직을, 생성기는 안정 셀 ID·순차 실행·검증을 맡는다.
공식 소스 스냅샷은 별도로 보존하고 가이드 노트북에 복제하지 않는다.
...
리스팅 재현 모드는 `src/` 에 실행할 코드가 있는 챕터에만 만든다. 공식 소스 지도 모드는
전 장에 자체 경량 실험이 있으면 예외다. 파일명은 `<책챕터번호>_chapter_guide.ipynb`.
