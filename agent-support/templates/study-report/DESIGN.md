...
- 기준 템플릿: `restful3/ai-odyssey-publisher` 의 theme_report.css, report.js, report HTML 구조.
...
4. 필요한 보충 — 원본 설명 근처에 추가 예제나 해석을 구분해 배치하며, 정해진 보충 목차를 채우지 않는다.
...
- 핵심 주장과 원리·예제·조건이 이어지는 설명 단락(단락 수로 완료를 판정하지 않음)
- 표 또는 도형
...
캡션은 리포트 자체의 `그림 N`과 관계가 드러나는 제목으로 시작한다. 원본 좌표와 재구성 범위는 가까운 출처 메모에 간결하게 보존하되 `교재 그림 N`, `AIML Quant 재구성` 같은 문구를 캡션마다 반복하지 않는다. 그림을 읽는 핵심 방향은 한두 문장으로 두고, 긴 해석과 결론은 같은 절의 일반 본문으로 옮긴다.
...
- 리포트가 재구성한 그림·표는 원본 좌표와 첫 완결 문장인 정확한 제목의 의미를 근접 출처에 보존하고, 리포트 화면에서는 자체 `그림 N`, `표 N` 번호를 사용한다. 뒤의 설명 문장은 그대로 전재하지 않고 자체 해설로 다시 쓴다. 실제로 추가한 해석·운영 규칙만 `AIML Quant 보충`으로 구분한다.
- 각 핵심 주장은 원서의 필요한 중간 설명, 사례·근거, 조건과 함께 이해되어야 한다. 해당 없는 요소를 임의로 만들지 않는다.
- 낯선 알고리즘은 질문·입출력 도해나 작은 계산 예·작동 원리·최소 코드·기록된 실행 결과·해석과 실패 경계가 이어져야 한다. 노트북에 결과 차트나 표가 있으면 코드와 함께 설명하고, 없는 결과를 꾸며 내지 않는다.
- 수식은 계산 대상과 모든 기호를 가까운 본문에서 풀어 쓰며, 약어와 단위는 첫 등장에 원어·한국어·실제 크기를 함께 설명한다. 정확도·수익률·위험조정 수익률·비용 차감 결과를 구분한다.
- Python 적응 문서는 Python API와 실행 의미를 기준으로 쓰고, 결과 해석에 필요하지 않은 MATLAB 등 원본 언어의 매개변수명과 이식 과정은 노출하지 않는다. 필요한 구현 차이는 코드 캡션이 아닌 일반 본문에서 설명한다.
- 시각자료는 원본 자산과 설명할 관계·비교·계산에 대응한다. 개수가 아닌 학습 기여와 실제 가독성으로 판정한다. 원자료에 없는 측정 차트를 만들지 않는다.
