...
SVG는 차트·관계도·플로우·타임라인에만 사용한다. 사람·장식 아이콘·일러스트를 그리지 않는다. 책의 그림을 그대로 베끼지 말고 논리를 새로 배치한 발표용 도해를 만든다. 외부 SVG의 기본 한글 font stack은 `report.css`의 `--font-sans-ko`와 같은 순서로 직접 선언한다.

### 그림 안에 두는 텍스트

**그림을 해독하는 텍스트인가, 설명하는 텍스트인가** 로 가른다. 해독 텍스트는 남긴다 — 그림을 슬라이드로 가져다 쓸 때도 필요하다.
...
**눈으로 훑으면 넘어가는 다섯 형태** (모두 실제로 배포된 적이 있다):

1. 제목처럼 보이지 않는 **본문 크기 한 줄 제목** (12.5px)
2. 가장자리가 아니라 **다이어그램 중앙에 놓인 콜아웃**
3. 계열 라벨의 `— 설명` 꼬리 (`학습셋 — 빠르게 오른다`)
4. 플롯 영역 위에 **떠 있는 결과 박스** (연결선이 닿지 않는다)
5. 다이어그램 우측에 붙은 **설명 패널**

빼기 전에 **figure 블록이 아니라 주변 절 전체를 검색한다.** 잘 쓴 리포트는 이미 그 내용을 본문에 담고 있으므로 기본 동작은 '옮기기'가 아니라 **'삭제'** 다.

이 규칙은 권고가 아니라 게이트다. `validate-site.py` 가 도형 밖 텍스트와 같은 x 로 쌓인 묶음을 28자 미만으로 제한한다. `agent-support/reviews/svg-prose-baseline.json` 은 이미 배포된 회차만 유예하므로, 회차를 정리하면 항목을 지우고 새 작업에는 절대 추가하지 않는다.
