...
def check_notebook(study: Study, path: Path, check_urls: bool = True) -> list[str]:
    ...
    # clean clone에서는 study.toml의 shared 자원이 아직 없다. setup_env.py가
    # upstream을 clone한 뒤 이 자원들을 교재 루트에 배치하므로, upstream 자체가
    # 없는 초기 상태에서만 선언된 shared 경로를 허용한다. bootstrap 이후에도
    # 빠져 있다면 실제 준비 오류이므로 평소처럼 실패한다.
    for text, target in re.findall(r"\[([^\]]*)\]\((?!https?:|attachment:|#)([^)]+)\)", markdown):
        resolved = (path.parent / urllib.parse.unquote(target)).resolve()
        shared_roots = tuple((study.root / name).resolve() for name in study.shared)
        provisioned_later = (
            not study.upstream.exists()
            and any(resolved == root or resolved.is_relative_to(root) for root in shared_roots)
        )
        if not resolved.exists() and not provisioned_later:
