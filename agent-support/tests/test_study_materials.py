...
CI가 html/와 agent-support/만 sparse checkout 하면 materials/가 없다.
...
def discover_studies() -> list[config.Study]:
    """분야별 active/archive에서 study.toml이 있는 교재를 모은다."""
    studies = []
    for track in ("aiml", "quant"):
        for state in ("active", "archive"):
            root = REPO_ROOT / "materials" / track / state
            if not root.is_dir():
                continue
...
skip_without_study = unittest.skipIf(
    not STUDIES,
    "study.toml이 있는 교재가 없다 (sparse checkout)",
)
