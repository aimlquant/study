...
def find_book() -> Path | None:
    """AI/ML active 아래에서 전환이 필요한 교재 루트를 찾는다."""
    active = REPO_ROOT / "materials" / "aiml" / "active"
    if not active.is_dir():
        return None
    return next(iter(sorted(active.glob(f"*/{BOOK_MARKER}"))), None)


BOOK = find_book()
BOOK_ROOT = BOOK.parent if BOOK is not None else None
# 교재가 archive로 옮겨가면 skip하고, active에 있으면 스크립트 부재를 실패로 잡는다.
SCRIPT = BOOK_ROOT / SCRIPT_NAME if BOOK_ROOT is not None else None


@unittest.skipIf(BOOK_ROOT is None, "전환이 필요한 교재가 AIML active에 없다")
