...
class RenumberResidueTests(unittest.TestCase):
    ...
    def test_missing_declared_shared_link_passes_before_bootstrap(self) -> None:
        path = write_notebook(
            self.study,
            markdown="[설정](../../../config.ini) · [데이터](../../../data/sample.json)",
        )
        self.assertEqual([], verify.check_notebook(self.study, path, check_urls=False))

    def test_missing_unmanaged_link_still_fails(self) -> None:
        path = write_notebook(self.study, markdown="[누락](../../../other.ini)")
        failures = verify.check_notebook(self.study, path, check_urls=False)
        self.assertTrue(any("other.ini" in failure for failure in failures), failures)
